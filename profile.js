...
export function initProfile({changed}){
 ...
 function preview(){const image=$('#f-avatar-preview');image.hidden=!avatar;if(avatar)image.src=avatar;else image.removeAttribute('src');$('#f-avatar-remove').hidden=!avatar;}
 $('#f-avatar').onchange=async()=>{
  ...
  }catch(e){if(current===version){$('#f-avatar').value='';imageError=e.message;$('#f-avatar-remove').hidden=false;$('#f-avatar-status').textContent=e.message;}}
